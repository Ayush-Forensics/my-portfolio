...
export const LimelightNav = ({
  ...
  return (
    <nav className={`relative inline-flex items-center h-16 rounded-lg bg-card text-foreground border px-2 ${className}`}>
          ...
          >
            {cloneElement(icon as React.ReactElement<any>, {
              className: `w-6 h-6 transition-opacity duration-100 ease-in-out ${
                activeIndex === index ? 'opacity-100' : 'opacity-40 group-hover:opacity-80'
              } ${(icon.props as any).className || ''} ${iconClassName || ''}`,
            })}
            
            {/* Display label dynamically when active or hovering if label is defined */}
            {label && (
                <span className={`absolute -bottom-8 whitespace-nowrap text-xs font-mono transition-opacity duration-200 ${activeIndex === index ? 'opacity-100 text-accent-primary' : 'opacity-0 group-hover:opacity-70'}`}>
                    {label}
                </span>
            )}
          </a>
      ...
      <div 
        ref={limelightRef}
        className={`absolute top-0 z-10 w-11 h-[5px] rounded-full bg-accent-primary shadow-[0_30px_15px_rgba(0,229,255,0.4)] ${
          isReady ? 'transition-[left] duration-400 ease-in-out' : ''
        } ${limelightClassName}`}
        style={{ left: '-999px' }}
      >
        <div className="absolute left-[-30%] top-[5px] w-[160%] h-14 [clip-path:polygon(5%_100%,25%_0,75%_0,95%_100%)] bg-gradient-to-b from-accent-primary/30 to-transparent pointer-events-none" />
      </div>
    </nav>
  );
};
